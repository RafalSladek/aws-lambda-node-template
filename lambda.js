...
exports.run = async (params, event, context, callback) => {
  ...
  const filenamesInfo = fileHandler.getFilenamesInfo(event, params.path);
  ...
  return Promise.all(requests)
    ...
    .catch(err => {
      if (params.shoudlFallback) {
        ...
        console.log(
          "Fallback due to {0} with {1}".format(
            err,
            filenamesInfo.originalFilename
          )
        );
        return {
          status: "302",
        ...
        createResponse(
          "401",
          "Bad request",
          "Error in processing: {0}. {1}".format(
            filenamesInfo.originalFilename,
            err
          )
        );
